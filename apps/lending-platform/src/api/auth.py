...
class AuthHandler:
    ...
    async def validate_token_simple(self, token: str) -> Optional[Dict[str, Any]]:
        """Simple token validation for WebSocket connections"""
        try:
            # For now, use the same JWT verification as the main verify_token method
...
async def get_auditor_user(user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    """Dependency to get auditor user"""
    if not auth_handler.has_any_role(user, ["auditor", "compliance_officer", "admin"]):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_compliance_user(user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    """Dependency to get compliance user"""
    if not auth_handler.has_any_role(user, ["compliance_officer", "auditor", "admin"]):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_developer_user(user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    """Dependency to get developer user"""
    if not auth_handler.has_any_role(user, ["developer", "admin"]):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def create_demo_token(role: str = "borrower") -> str:
    """Create a demo token for testing"""
    role_configs = {
        "borrower": {
            "user_id": "demo_borrower_123",
            "email": "borrower@example.com",
            "algorand_address": "7ZUECA7HFLZTXENRV24SHLU4AVPUTMTTDUFUBNBD64C73F3UHRTHAIOF6Q",
            "roles": ["borrower"]
        },
        "lender": {
            "user_id": "demo_lender_456",
            "email": "lender@example.com",
            "algorand_address": "LENDERWALLETSAMPLEADDRESSHEREFORALGORANDUSE654321",
            "roles": ["lender"]
        },
        "auditor": {
            "user_id": "demo_auditor_789",
            "email": "auditor@example.com",
            "algorand_address": "AUDITORDEMOWALLETADDRESSHEREFORALGORANDUSE987654",
            "roles": ["auditor"]
        },
        "compliance_officer": {
            "user_id": "demo_compliance_101",
            "email": "compliance@example.com",
            "algorand_address": "COMPLIANCEDEMOWALLETADDRESSHEREFORALGORAND101112",
            "roles": ["compliance_officer"]
        },
        "admin": {
            "user_id": "demo_admin_999",
            "email": "admin@example.com",
            "algorand_address": "ADMINDEMOWALLETADDRESSHEREFORALGORANDUSE999888",
            "roles": ["admin", "auditor", "compliance_officer", "lender"]
        },
        "developer": {
            "user_id": "demo_developer_555",
            "email": "developer@example.com",
            "algorand_address": "DEVELOPERDEMOWALLETADDRESSHEREFORALGORAND555444",
            "roles": ["developer", "auditor"]
        }
    }

    demo_user = role_configs.get(role, role_configs["borrower"])
