...
For quick local testing without secrets, you can run the worker in read-only mode which disables OAuth/Vault and any wallet/transaction tools. It exposes an SSE endpoint at `/sse` with only read-safe tools (api_*, knowledge where available, and utilities).

Steps:

1. Install deps: `npm install`
2. Start dev server: `npx wrangler dev --port 8787`
3. Connect MCP client: `npx mcp-remote http://localhost:8787/sse`
...
Note: Read-only mode is enabled via `READ_ONLY=true` in `wrangler.jsonc` vars and skips OAuth/Vault bindings.
